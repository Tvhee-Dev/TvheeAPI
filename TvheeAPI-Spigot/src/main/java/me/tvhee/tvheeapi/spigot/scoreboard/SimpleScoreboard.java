package me.tvhee.tvheeapi.spigot.scoreboard;
...
public class SimpleScoreboard
{
	...
	public SimpleScoreboard(Component title)
	{
		this(title, 20);
	}
	 ...
	 */
	public SimpleScoreboard(Component title, int updateDelayTicks)
	{
		this.setTitle(title);
	...
	{
		return new ArrayList<>(registeredBoards);
	}
	 ...
	 * @return
	 */
	protected Component replaceVariables(Player player, Component message)
	{
		return message;
	}
	...
	{
		int maxTitleLength = MCReflectionUtil.versionAtLeast("1_13_R1") ? 128 : 32;
		this.title = !title.isNullOrEmpty() && title.getText().length() > maxTitleLength ?
			new Component(title.getText().substring(0, maxTitleLength)) : title;
		this.title = !title.isNullOrEmpty() && this.title.getText().endsWith(COLOR_CHAR) ?
			new Component(this.title.getText().substring(0, this.title.getText().length() - 1)) : this.title;
	}

	public List<Component> getRows()
	{
		return CollectionUtil.convertList(rows, value -> new Component(value.text));
	}
	...
	{
		if(this.title == null || this.title.isNullOrEmpty())
			throw new TvheeAPIException(getClass(), "show", "Before calling show(Player) you need to use setTitle() for " + this);

		if(this.isViewing(player))
			throw new TvheeAPIException(getClass(), "show", "Player " + player.getName() + " is already viewing scoreboard: " + this);
	...
	{
		if(!this.isViewing(player))
			throw new TvheeAPIException(getClass(), "show", "Player " + player.getName() + " is not viewing scoreboard: " + this);
	 ...
	 * @param secondary
	 */
	public final void setTheme(ChatColor primary, ChatColor secondary)
	{
		if(secondary != null)
		{
	...
	{
		if((this.rows.size() + entries.size()) > 17)
			throw new TvheeAPIException(getClass(), "addRows", "You are trying to add too many rows (the limit is 16)");


		this.rows.addAll(CollectionUtil.convertList(CollectionUtil.convertList(entries, Object::toString), ScoreboardLine::new));
	}

	public final void changeRow(int index, Object value)
	{
		if(index > this.rows.size())
			throw new TvheeAPIException(getClass(), "changeRow", "The row for index " + index + " is currently not existing. Please use addRows()!");

		this.rows.get(index).setText(value.toString());
	}
	...
	{
		if(updateTask != null)
			throw new TvheeAPIException(getClass(), "start", "Scoreboard " + this + " already running");

		this.updateTask = Bukkit.getScheduler().runTaskTimer((Plugin) TvheeAPIPlugin.getPlugin(), () ->
		{
			try
			{
				this.onUpdate();

				for(UUID viewerId : new ArrayList<>(this.viewers))
				{
	 ...
	 * @return
	 */
	private Component replaceTheme(Component row)
	{
		if(!row.isNullOrEmpty() && row.getText().contains(":"))
		{
			if(this.theme.length == 1)
			{
				return new Component(this.theme[0] + row);
			}
			else if(this.theme[0] != null)
			{
				...
				if(split.length > 1)
					return new Component(this.theme[0] + split[0] + ":" + this.theme[1] + split[1]);
			}
		}
	...
	private void reloadEntries(Player player) throws IllegalArgumentException
	{
		...
		if(mainboard == null)
		{
			...
			mainboard.setDisplayName(title.toLegacyText());
			mainboard.setDisplaySlot(DisplaySlot.SIDEBAR);
		}

		if(!mainboard.getDisplayName().equals(title.toLegacyText()))
			mainboard.setDisplayName(title.toLegacyText());
		...
		for(int lineNumber = 0; lineNumber < 16; lineNumber++)
		{
			...
			if(lineNumber < this.rows.size())
			{
				...
				final Function<String, String> replaceFunction =
						(lineText) -> this.replaceTheme(this.replaceVariables(player, new Component(lineText))).toLegacyText();
	...
	private static class ScoreboardLine
	{
		...
		public String[] createLine(String lineIdentifier, Function<String, String> textReplaceFunction)
		{
			...
			final boolean mc1_13 = MCReflectionUtil.versionAtLeast("v1_13_R1");
			final boolean mc1_18 = MCReflectionUtil.versionAtLeast("v1_18_R1");
			...
			line[0] = copy.isEmpty() ? "" : copy.get(0); //Team Prefix
			line[1] = copy.size() < 2 ? lineIdentifier : copy.get(1); //Score entry
			line[2] = copy.size() < 3 ? "" : copy.get(2); //Team Suffix
